public class S5HLargestSubMatrixSum {
    /*
    Sum((k,t), (i, j)) = prefixSum[i][j] - prefixSum[k-1][i] - prefixSum[i][t-1] + prefixSum[k-1][t-1]
    M[i][j] represents the 2D prefix sum from (0,0) to (i, j)
    preprocess time = O(n^2)

    for for for for (i, j, k, t) {
            Sum((k,t), (i, j)) = prefixSum[i][j] - prefixSum[k-1][i] - prefixSum[i][t-1] + prefixSum[k-1][t-1]
    }
    get the global max
